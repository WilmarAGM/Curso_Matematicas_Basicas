...
import { MODULES_SEMANA9, SYSTEMS_APPLICATION_QUIZ, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'

interface Module45Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module45SystemsApplications({ onNavigate, onDone }: Module45Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = SYSTEMS_APPLICATION_QUIZ[current]
  const isLast = current === SYSTEMS_APPLICATION_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      ...
      hubId="semana9"
      hubLabel="Semana 9"
      intro="Cierra la Semana 9 modelando situaciones reales — números, inversiones y circuitos eléctricos — con sistemas de ecuaciones 2x2 y 3x3."
    >
      <div className="mb-4 flex items-baseline justify-between">
        <h2 className="font-display text-xl text-ink">
          Problema {Math.min(current + 1, SYSTEMS_APPLICATION_QUIZ.length)}/{SYSTEMS_APPLICATION_QUIZ.length}
        </h2>
      </div>
      ...
      {!finished ? (
        ...
        <div className="rounded border border-line bg-surface p-6 text-center">
          <p className="font-display text-2xl text-ink">
            Obtuviste {score} de {SYSTEMS_APPLICATION_QUIZ.length}
          </p>
          <p className="mt-1 text-sm text-ink-muted">
            {score === SYSTEMS_APPLICATION_QUIZ.length
              ? '¡Excelente! Completaste la Semana 9 con un puntaje perfecto.'
