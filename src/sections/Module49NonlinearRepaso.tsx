...
import { MODULES_SEMANA10, NONLINEAR_REPASO_QUIZ, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'

interface Module49Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module49NonlinearRepaso({ onNavigate, onDone }: Module49Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = NONLINEAR_REPASO_QUIZ[current]
  const isLast = current === NONLINEAR_REPASO_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      id="noLinealRepaso"
      eyebrow="Módulo 04"
      title="Repaso Mixto: Los Tres Casos"
      onNavigate={onNavigate}
      modules={MODULES_SEMANA10}
      hubId="semana10"
      hubLabel="Semana 10"
      intro="Reconoce cuál de los tres casos aplica en cada sistema y elige el método más eficiente: sustitución, reducción, o factorización."
    >
      <div className="mb-4 flex items-baseline justify-between">
        <h2 className="font-display text-xl text-ink">
          Problema {Math.min(current + 1, NONLINEAR_REPASO_QUIZ.length)}/{NONLINEAR_REPASO_QUIZ.length}
        </h2>
      </div>
      ...
      {!finished ? (
        ...
        <div className="rounded border border-line bg-surface p-6 text-center">
          <p className="font-display text-2xl text-ink">
            Obtuviste {score} de {NONLINEAR_REPASO_QUIZ.length}
          </p>
          <p className="mt-1 text-sm text-ink-muted">
            {score === NONLINEAR_REPASO_QUIZ.length
              ? 'Excelente, dominas los tres casos de sistemas no lineales.'
