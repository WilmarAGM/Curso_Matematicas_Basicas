...
import {
  MODULES_SEMANA9,
  SYS_2X2_CRAMER_STEPS,
  SYS_2X2_METHOD2_QUIZ,
  SYS_2X2_REDUCTION_STEPS,
  type SectionId,
} from '@/data/content'
import { cn } from '@/lib/utils'

interface Module42Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module42Sys2x2Cramer({ onNavigate, onDone }: Module42Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = SYS_2X2_METHOD2_QUIZ[current]
  const isLast = current === SYS_2X2_METHOD2_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      id="sistema2x2ReduccionCramer"
      eyebrow="Módulo 02"
      title="Sistema 2x2: Reducción y Cramer"
      onNavigate={onNavigate}
      modules={MODULES_SEMANA9}
      hubId="semana9"
      hubLabel="Semana 9"
      intro="Reducción (eliminación) busca cancelar una variable sumando las ecuaciones. Cramer usa determinantes: Ds del sistema, y Dx, Dy reemplazando cada columna por los términos independientes."
    >
      <div className="grid gap-6 lg:grid-cols-2">
        <StepBlock title="Reducción o eliminación" steps={SYS_2X2_REDUCTION_STEPS} accent="pine" />
        <StepBlock title="Cramer (determinantes)" steps={SYS_2X2_CRAMER_STEPS} accent="ember" />
      </div>
      ...
      <div className="mt-12">
        <div className="mb-4 flex items-baseline justify-between">
          ...
          <span className="font-mono-nums text-sm text-ink-muted">
            Problema {Math.min(current + 1, SYS_2X2_METHOD2_QUIZ.length)}/{SYS_2X2_METHOD2_QUIZ.length}
          </span>
        </div>
        ...
        {!finished ? (
          ...
          <div className="rounded border border-line bg-surface p-6 text-center">
            <p className="font-display text-2xl text-ink">
              Obtuviste {score} de {SYS_2X2_METHOD2_QUIZ.length}
            </p>
            <p className="mt-1 text-sm text-ink-muted">
              {score === SYS_2X2_METHOD2_QUIZ.length
                ? 'Excelente, dominas reducción y Cramer.'
