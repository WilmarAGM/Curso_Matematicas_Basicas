...
import { FRAC_PROPERTY_LIST, FRAC_PROPERTY_QUIZ, MODULES_SEMANA2, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'

const OPTIONS = ['Igualdad', 'Fracción negativa', 'Multiplicación', 'Simplificación', 'Suma y resta', 'División'] as const
...
export function Module7FracProps({ onNavigate, onDone }: Module7Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<string | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = FRAC_PROPERTY_QUIZ[current]
  const isLast = current === FRAC_PROPERTY_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      id="propiedadesFracciones"
      eyebrow="Módulo 02"
      title="Propiedades de las Fracciones"
      onNavigate={onNavigate}
      modules={MODULES_SEMANA2}
      hubId="semana2"
      hubLabel="Semana 2"
      intro="Los fraccionarios (a/b, con b ≠ 0) cumplen propiedades para operarlos con seguridad: igualdad, fracción negativa, simplificación y las cuatro operaciones."
    >
      <div className="grid gap-3 sm:grid-cols-2">
        {FRAC_PROPERTY_LIST.map((p) => (
      ...
      <div className="mt-12">
        <div className="mb-4 flex items-baseline justify-between">
          ...
          <span className="font-mono-nums text-sm text-ink-muted">
            Pregunta {Math.min(current + 1, FRAC_PROPERTY_QUIZ.length)}/{FRAC_PROPERTY_QUIZ.length}
          </span>
        </div>
        ...
        {!finished ? (
          ...
          <div className="rounded border border-line bg-surface p-6 text-center">
            <p className="font-display text-2xl text-ink">
              Obtuviste {score} de {FRAC_PROPERTY_QUIZ.length}
            </p>
            <p className="mt-1 text-sm text-ink-muted">
              {score === FRAC_PROPERTY_QUIZ.length
                ? 'Excelente, dominas las propiedades de las fracciones.'
