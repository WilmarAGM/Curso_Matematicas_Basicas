...
import { MODULES_SEMANA10, NONLINEAR_CASE3_QUIZ, NONLINEAR_CASE3_STEPS, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'

interface Module48Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module48NonlinearCase3({ onNavigate, onDone }: Module48Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = NONLINEAR_CASE3_QUIZ[current]
  const isLast = current === NONLINEAR_CASE3_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      id="noLinealCaso3"
      eyebrow="Módulo 03"
      title="Caso 3: Una Ecuación Lineal y Una Irracional"
      onNavigate={onNavigate}
      modules={MODULES_SEMANA10}
      hubId="semana10"
      hubLabel="Semana 10"
      intro="Cuando aparece un radical, se eleva al cuadrado para eliminarlo — pero esto puede introducir una solución extraña (falsa), así que siempre hay que comprobar cada respuesta en la ecuación original."
    >
      <StepBlock title="Ejemplo: 2√(x+1) = y+1, 2x − 3y = 1" steps={NONLINEAR_CASE3_STEPS} accent="ember" />

      <div className="mt-8 rounded border border-warm/40 bg-warm/5 p-4 text-sm text-ink-muted">
        ⚠️ <strong className="text-ink">Solución extraña:</strong> al elevar al cuadrado, y=−1 también
        resolvía la ecuación transformada, pero no cumple la ecuación original con la raíz (el lado
        izquierdo 2√(x+1) nunca puede ser negativo). Por eso se descarta.
      </div>
      ...
      <div className="mt-12">
        <div className="mb-4 flex items-baseline justify-between">
          ...
          <span className="font-mono-nums text-sm text-ink-muted">
            Problema {Math.min(current + 1, NONLINEAR_CASE3_QUIZ.length)}/{NONLINEAR_CASE3_QUIZ.length}
          </span>
        </div>
        ...
        {!finished ? (
          ...
          <div className="rounded border border-line bg-surface p-6 text-center">
            <p className="font-display text-2xl text-ink">
              Obtuviste {score} de {NONLINEAR_CASE3_QUIZ.length}
            </p>
            <button
              onClick={() => {
                setCurrent(0)
