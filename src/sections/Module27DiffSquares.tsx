...
import { DifferenceOfSquaresTiles } from '@/components/app/AlgebraTiles'
import { DIFF_SQ_QUIZ, DIFF_SQ_STEPS, DIFF_SQ_STEPS_2, MODULES_SEMANA6, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'

interface Module27Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module27DiffSquares({ onNavigate, onDone }: Module27Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = DIFF_SQ_QUIZ[current]
  const isLast = current === DIFF_SQ_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      ...
      hubId="semana6"
      hubLabel="Semana 6"
      intro="El resultado de una diferencia de cuadrados obedece al producto de la suma por la diferencia de las raíces cuadradas de cada término: a² − b² = (a + b)(a − b). Ya viste la prueba geométrica en Semana 4 — aquí la usamos para factorizar."
    >
      <div className="rounded border border-line bg-surface p-6">
        <h2 className="font-display text-lg text-ink">Recordatorio geométrico</h2>
        <div className="mt-5">
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-2">
        <StepBlock title="Ejemplo: 4x² − 9y²" steps={DIFF_SQ_STEPS} accent="pine" />
        <StepBlock title="Ejemplo: 36m² − 25n²" steps={DIFF_SQ_STEPS_2} accent="leaf" />
      </div>
      ...
      <div className="mt-12">
        <div className="mb-4 flex items-baseline justify-between">
          ...
          <span className="font-mono-nums text-sm text-ink-muted">
            Problema {Math.min(current + 1, DIFF_SQ_QUIZ.length)}/{DIFF_SQ_QUIZ.length}
          </span>
        </div>
        ...
        {!finished ? (
          ...
          <div className="rounded border border-line bg-surface p-6 text-center">
            <p className="font-display text-2xl text-ink">
              Obtuviste {score} de {DIFF_SQ_QUIZ.length}
            </p>
            <p className="mt-1 text-sm text-ink-muted">
              {score === DIFF_SQ_QUIZ.length
                ? 'Excelente, dominas la diferencia de cuadrados.'
