...
import { EQUATIONS_APPLICATION_QUIZ, MODULES_SEMANA8, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'

interface Module40Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module40EquationsApplications({ onNavigate, onDone }: Module40Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = EQUATIONS_APPLICATION_QUIZ[current]
  const isLast = current === EQUATIONS_APPLICATION_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      ...
      hubId="semana8"
      hubLabel="Semana 8"
      intro="Cierra la Semana 8 modelando situaciones de geometría y movimiento (proyectiles, tanques, jardines) con ecuaciones cuadráticas."
    >
      <div className="mb-4 flex items-baseline justify-between">
        <h2 className="font-display text-xl text-ink">
          Problema {Math.min(current + 1, EQUATIONS_APPLICATION_QUIZ.length)}/{EQUATIONS_APPLICATION_QUIZ.length}
        </h2>
      </div>
      ...
      {!finished ? (
        ...
        <div className="rounded border border-line bg-surface p-6 text-center">
          <p className="font-display text-2xl text-ink">
            Obtuviste {score} de {EQUATIONS_APPLICATION_QUIZ.length}
          </p>
          <p className="mt-1 text-sm text-ink-muted">
            {score === EQUATIONS_APPLICATION_QUIZ.length
              ? '¡Excelente! Completaste la Semana 8 con un puntaje perfecto.'
