...
import { MODULES_SEMANA10, NONLINEAR_APPLICATION_QUIZ, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'

interface Module50Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module50NonlinearApplications({ onNavigate, onDone }: Module50Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = NONLINEAR_APPLICATION_QUIZ[current]
  const isLast = current === NONLINEAR_APPLICATION_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      ...
      hubId="semana10"
      hubLabel="Semana 10"
      intro="Cierra la Semana 10 (y el bloque completo de sistemas de ecuaciones) modelando situaciones de dinero, personas y movimiento con sistemas de dos ecuaciones."
    >
      <div className="mb-4 flex items-baseline justify-between">
        <h2 className="font-display text-xl text-ink">
          Problema {Math.min(current + 1, NONLINEAR_APPLICATION_QUIZ.length)}/{NONLINEAR_APPLICATION_QUIZ.length}
        </h2>
      </div>
      ...
      {!finished ? (
        ...
        <div className="rounded border border-line bg-surface p-6 text-center">
          <p className="font-display text-2xl text-ink">
            Obtuviste {score} de {NONLINEAR_APPLICATION_QUIZ.length}
          </p>
          <p className="mt-1 text-sm text-ink-muted">
            {score === NONLINEAR_APPLICATION_QUIZ.length
              ? '¡Excelente! Completaste la Semana 10 con un puntaje perfecto.'
