...
import { FACTOR_APPLICATION_QUIZ, MODULES_SEMANA7, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'

interface Module35Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module35FactorRationalApplications({ onNavigate, onDone }: Module35Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = FACTOR_APPLICATION_QUIZ[current]
  const isLast = current === FACTOR_APPLICATION_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      ...
      hubId="semana7"
      hubLabel="Semana 7"
      intro="Cierra la Semana 7 combinando teorema del factor, factorización y sistemas simples (suma y producto) para resolver problemas de geometría, cinemática y números."
    >
      <div className="mb-4 flex items-baseline justify-between">
        <h2 className="font-display text-xl text-ink">
          Problema {Math.min(current + 1, FACTOR_APPLICATION_QUIZ.length)}/{FACTOR_APPLICATION_QUIZ.length}
        </h2>
      </div>
      ...
      {!finished ? (
        ...
        <div className="rounded border border-line bg-surface p-6 text-center">
          <p className="font-display text-2xl text-ink">
            Obtuviste {score} de {FACTOR_APPLICATION_QUIZ.length}
          </p>
          <p className="mt-1 text-sm text-ink-muted">
            {score === FACTOR_APPLICATION_QUIZ.length
              ? '¡Excelente! Completaste la Semana 7 con un puntaje perfecto.'
