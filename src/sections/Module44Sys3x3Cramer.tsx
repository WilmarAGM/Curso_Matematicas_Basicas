...
import {
  MODULES_SEMANA9,
  SYS_3X3_CRAMER_STEPS,
  SYS_3X3_METHOD2_QUIZ,
  SYS_3X3_REDUCTION_STEPS,
  type SectionId,
} from '@/data/content'
import { cn } from '@/lib/utils'

interface Module44Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module44Sys3x3Cramer({ onNavigate, onDone }: Module44Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = SYS_3X3_METHOD2_QUIZ[current]
  const isLast = current === SYS_3X3_METHOD2_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      id="sistema3x3ReduccionCramer"
      eyebrow="Módulo 04"
      title="Sistema 3x3: Reducción y Cramer"
      onNavigate={onNavigate}
      modules={MODULES_SEMANA9}
      hubId="semana9"
      hubLabel="Semana 9"
      intro="En reducción, eliminamos una variable combinando pares de ecuaciones hasta llegar a un sistema 2x2. En Cramer, calculamos el determinante del sistema y uno por cada variable, reemplazando esa columna por los términos independientes."
    >
      <div className="grid gap-6 lg:grid-cols-2">
        <StepBlock title="Reducción o eliminación" steps={SYS_3X3_REDUCTION_STEPS} accent="pine" />
        <StepBlock title="Cramer (determinantes)" steps={SYS_3X3_CRAMER_STEPS} accent="ember" />
      </div>
      ...
      <div className="mt-12">
        <div className="mb-4 flex items-baseline justify-between">
          ...
          <span className="font-mono-nums text-sm text-ink-muted">
            Problema {Math.min(current + 1, SYS_3X3_METHOD2_QUIZ.length)}/{SYS_3X3_METHOD2_QUIZ.length}
          </span>
        </div>
        ...
        {!finished ? (
          ...
          <div className="rounded border border-line bg-surface p-6 text-center">
            <p className="font-display text-2xl text-ink">
              Obtuviste {score} de {SYS_3X3_METHOD2_QUIZ.length}
            </p>
            <p className="mt-1 text-sm text-ink-muted">
              {score === SYS_3X3_METHOD2_QUIZ.length
                ? 'Excelente, dominas reducción y Cramer en 3x3.'
