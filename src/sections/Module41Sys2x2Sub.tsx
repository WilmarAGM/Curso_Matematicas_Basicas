...
import { MODULES_SEMANA9, SYS_2X2_EQUALIZATION_STEPS, SYS_2X2_QUIZ, SYS_2X2_SUBSTITUTION_STEPS, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'

interface Module41Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module41Sys2x2Sub({ onNavigate, onDone }: Module41Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = SYS_2X2_QUIZ[current]
  const isLast = current === SYS_2X2_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      id="sistema2x2SustitucionIgualacion"
      eyebrow="Módulo 01"
      title="Sistema 2x2: Sustitución e Igualación"
      onNavigate={onNavigate}
      modules={MODULES_SEMANA9}
      hubId="semana9"
      hubLabel="Semana 9"
      prevFallback={{ id: 'semana9', label: 'Semana 9' }}
      intro="Un sistema 2x2 tiene dos incógnitas y dos ecuaciones. Existen varios métodos de solución; aquí vemos sustitución (despejar y reemplazar) e igualación (despejar la misma variable en ambas y igualar)."
    >
      <div className="grid gap-6 lg:grid-cols-2">
        <StepBlock title="Sustitución" steps={SYS_2X2_SUBSTITUTION_STEPS} accent="pine" />
        <StepBlock title="Igualación" steps={SYS_2X2_EQUALIZATION_STEPS} accent="leaf" />
      </div>

      <div className="mt-12">
        <div className="mb-4 flex items-baseline justify-between">
          <h2 className="font-display text-xl text-ink">Practica: resuelve el sistema 2x2</h2>
          <span className="font-mono-nums text-sm text-ink-muted">
            Problema {Math.min(current + 1, SYS_2X2_QUIZ.length)}/{SYS_2X2_QUIZ.length}
          </span>
        </div>
        ...
        {!finished ? (
          ...
          <div className="rounded border border-line bg-surface p-6 text-center">
            <p className="font-display text-2xl text-ink">
              Obtuviste {score} de {SYS_2X2_QUIZ.length}
            </p>
            <p className="mt-1 text-sm text-ink-muted">
              {score === SYS_2X2_QUIZ.length
                ? 'Excelente, dominas sustitución e igualación.'
