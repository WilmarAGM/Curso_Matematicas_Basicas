import { MODULES_SEMANA8, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'
import { Card } from '@/components/ui/card'

interface WeekHub8Props {
  onNavigate: (id: SectionId) => void
  completed: Record<string, boolean>
}

export function WeekHub8({ onNavigate, completed }: WeekHub8Props) {
  const doneCount = MODULES_SEMANA8.filter((m) => completed[m.id]).length

  return (
    <div className="mx-auto max-w-6xl px-5 py-12">
      <p className="font-mono-nums text-sm uppercase tracking-[0.14em] text-warm">Semana 8</p>
      <h1 className="mt-2 font-display text-4xl text-ink">Ecuaciones Lineales y Cuadráticas</h1>
      <p className="mt-3 max-w-prose text-[15px] leading-relaxed text-ink-muted">
        Recorre los cinco módulos: ecuación lineal, ecuación cuadrática, discriminante y números
        complejos, operaciones con complejos, y aplicaciones. Cada uno combina teoría con un ejercicio
      ...
      <div className="mt-4 flex items-center gap-3">
        <div className="h-1.5 w-48 overflow-hidden rounded-full bg-line">
          <div
            ...
            style={{ width: `${(doneCount / MODULES_SEMANA8.length) * 100}%` }}
          />
        </div>
        <span className="font-mono-nums text-sm text-ink-muted">
          {doneCount}/{MODULES_SEMANA8.length} completados
        </span>
      </div>

      <div className="mt-10 grid gap-4 sm:grid-cols-2">
        {MODULES_SEMANA8.map((m) => (
          <Card
            key={m.id}
            role="button"
