...
import { MODULES_SEMANA7, RATIONALIZE_DEN_STEPS, RATIONALIZE_NUM_STEPS, RATIONALIZE_QUIZ, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'

interface Module34Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module34Rationalization({ onNavigate, onDone }: Module34Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = RATIONALIZE_QUIZ[current]
  const isLast = current === RATIONALIZE_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      ...
      hubId="semana7"
      hubLabel="Semana 7"
      intro="Racionalizar consiste en eliminar los radicales del numerador o del denominador, multiplicando por la conjugada — la misma expresión con el signo del medio cambiado — para convertirla en una diferencia de cuadrados."
    >
      <div className="grid gap-6 lg:grid-cols-2">
        <StepBlock title="Racionalización de numerador: (√(x+h) − √x)/h" steps={RATIONALIZE_NUM_STEPS} accent="pine" />
        <StepBlock title="Racionalización de denominador" steps={RATIONALIZE_DEN_STEPS} accent="leaf" />
      </div>
      <div className="mt-8 rounded border border-line bg-surface-raised/40 p-4 text-sm text-ink-muted">
        💡 <strong className="text-ink">Idea clave:</strong> la conjugada de (a − b) es (a + b). Al
        multiplicar (a−b)(a+b) siempre obtienes a² − b², que elimina las raíces cuadradas.
      </div>

      <div className="mt-12">
        <div className="mb-4 flex items-baseline justify-between">
          <h2 className="font-display text-xl text-ink">Practica: racionaliza</h2>
          <span className="font-mono-nums text-sm text-ink-muted">
            Problema {Math.min(current + 1, RATIONALIZE_QUIZ.length)}/{RATIONALIZE_QUIZ.length}
          </span>
        </div>
        ...
        {!finished ? (
          ...
          <div className="rounded border border-line bg-surface p-6 text-center">
            <p className="font-display text-2xl text-ink">
              Obtuviste {score} de {RATIONALIZE_QUIZ.length}
            </p>
            <p className="mt-1 text-sm text-ink-muted">
              {score === RATIONALIZE_QUIZ.length
                ? 'Excelente, dominas la racionalización.'
