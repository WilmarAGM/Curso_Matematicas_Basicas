...
import { MODULES_SEMANA5, TRINOMIO_GENERAL_QUIZ, TRINOMIO_GENERAL_STEPS, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'

interface Module24Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module24TrinomioGeneral({ onNavigate, onDone }: Module24Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = TRINOMIO_GENERAL_QUIZ[current]
  const isLast = current === TRINOMIO_GENERAL_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      ...
      intro={
        <>
          Igual que el caso anterior, pero ahora también descomponemos el coeficiente <strong>a</strong>{' '}
          en sus factores. Se prueba por ensayo y error hasta que el producto cruzado de los binomios
          reproduzca el término medio.
        </>
      }
    >
      <StepBlock title="Ejemplo: 6x² + 7x − 5" steps={TRINOMIO_GENERAL_STEPS} accent="ember" />

      <div className="mt-8 rounded border border-line bg-surface-raised/40 p-4 text-sm text-ink-muted">
        💡 <strong className="text-ink">Tip:</strong> organiza las pruebas como una tabla — factores de a
        en una columna, factores de c en la otra — y verifica el producto cruzado antes de aceptar una
        combinación.
      </div>
      ...
      <div className="mt-12">
        <div className="mb-4 flex items-baseline justify-between">
          ...
          <span className="font-mono-nums text-sm text-ink-muted">
            Problema {Math.min(current + 1, TRINOMIO_GENERAL_QUIZ.length)}/{TRINOMIO_GENERAL_QUIZ.length}
          </span>
        </div>
        ...
        {!finished ? (
          ...
          <div className="rounded border border-line bg-surface p-6 text-center">
            <p className="font-display text-2xl text-ink">
              Obtuviste {score} de {TRINOMIO_GENERAL_QUIZ.length}
            </p>
            <p className="mt-1 text-sm text-ink-muted">
              {score === TRINOMIO_GENERAL_QUIZ.length
                ? 'Excelente, dominas el trinomio ax² + bx + c.'
