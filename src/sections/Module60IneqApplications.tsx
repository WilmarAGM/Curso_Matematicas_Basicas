...
import { INEQ_APPLICATION_QUIZ, MODULES_SEMANA12, type SectionId } from '@/data/content'
import { cn } from '@/lib/utils'

interface Module60Props {
  onNavigate: (id: SectionId) => void
  onDone: () => void
}

export function Module60IneqApplications({ onNavigate, onDone }: Module60Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = INEQ_APPLICATION_QUIZ[current]
  const isLast = current === INEQ_APPLICATION_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      ...
      hubId="semana12"
      hubLabel="Semana 12"
      intro="Cierra la Semana 12 — y el bloque completo de Álgebra— modelando presupuestos, dietas y niveles de producción con inecuaciones."
    >
      <div className="mb-4 flex items-baseline justify-between">
        <h2 className="font-display text-xl text-ink">
          Problema {Math.min(current + 1, INEQ_APPLICATION_QUIZ.length)}/{INEQ_APPLICATION_QUIZ.length}
        </h2>
      </div>
      ...
      {!finished ? (
        ...
        <div className="rounded border border-line bg-surface p-6 text-center">
          <p className="font-display text-2xl text-ink">
            Obtuviste {score} de {INEQ_APPLICATION_QUIZ.length}
          </p>
          <p className="mt-1 text-sm text-ink-muted">
            {score === INEQ_APPLICATION_QUIZ.length
              ? '¡Excelente! Completaste toda la unidad de Álgebra (Semanas 4-12) con un puntaje perfecto.'
              : '¡Completaste toda la unidad de Álgebra (Semanas 4-12)! Revisa las soluciones si quieres mejorar tu puntaje.'}
          </p>
          <button
            onClick={() => {
              setCurrent(0)
