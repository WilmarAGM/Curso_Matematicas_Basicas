...
import { FACTOR_THEOREM_INTRO, FACTOR_THEOREM_QUIZ, FACTOR_THEOREM_STEPS, MODULES_SEMANA7, type SectionId } from '@/data/content'
...
export function Module32FactorTheorem({ onNavigate, onDone }: Module32Props) {
  const [current, setCurrent] = useState(0)
  const [choice, setChoice] = useState<number | null>(null)
  const [score, setScore] = useState(0)
  const [finished, setFinished] = useState(false)

  const q = FACTOR_THEOREM_QUIZ[current]
  const isLast = current === FACTOR_THEOREM_QUIZ.length - 1
  ...
  return (
    <ModuleShell
      id="teoremaFactor"
      eyebrow="Módulo 02"
      title="Teorema del Factor"
      onNavigate={onNavigate}
      modules={MODULES_SEMANA7}
      hubId="semana7"
      hubLabel="Semana 7"
      intro={FACTOR_THEOREM_INTRO}
    >
      <StepBlock title="Ejemplo: factoriza P(x) = 2x³ + x² − 13x + 6" steps={FACTOR_THEOREM_STEPS} accent="ember" />

      <div className="mt-8 rounded border border-line bg-surface-raised/40 p-4 text-sm text-ink-muted">
        💡 <strong className="text-ink">Estrategia:</strong> los posibles ceros racionales de un
        polinomio son (factores del término constante) / (factores del coeficiente principal). Prueba
        esos valores hasta encontrar uno que anule el polinomio — ese será tu primer factor.
      </div>

      <div className="mt-12">
        <div className="mb-4 flex items-baseline justify-between">
          <h2 className="font-display text-xl text-ink">Practica: factoriza usando el teorema</h2>
          <span className="font-mono-nums text-sm text-ink-muted">
            Problema {Math.min(current + 1, FACTOR_THEOREM_QUIZ.length)}/{FACTOR_THEOREM_QUIZ.length}
          </span>
        </div>
        ...
        {!finished ? (
          ...
          <div className="rounded border border-line bg-surface p-6 text-center">
            <p className="font-display text-2xl text-ink">
              Obtuviste {score} de {FACTOR_THEOREM_QUIZ.length}
            </p>
            <p className="mt-1 text-sm text-ink-muted">
              {score === FACTOR_THEOREM_QUIZ.length
                ? 'Excelente, dominas el teorema del factor.'
